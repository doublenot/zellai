#!/bin/bash
# .yoyo/scripts/grow.sh — One growth session for the zellai project.
...
#   ALLOWED_AUTHORS    — Comma-separated GitHub logins for agent-input issues (default: doublenot)
...
PROTECTED_PATHS="zellai-vision.md SCHEMA.md YOYO.md .github/workflows/ .yoyo/scripts/ .yoyo/config.toml .yoyo/.gitignore .yoyo/skills/grow/ .yoyo/skills/communicate/ .yoyo/skills/research/"
...
if command -v gh &>/dev/null; then
    if ! gh issue list --repo "$REPO" \
        ...
        ALLOWED_AUTHORS="${ALLOWED_AUTHORS:-doublenot}" \
        BOT_SLUG="$BOT_SLUG" \
...
if [ -f Cargo.toml ]; then
    if BUILD_OUT=$(cargo build --target wasm32-wasip1 2>&1); then
        echo "  Build: PASS"
    else
    ...
    echo "  No Cargo.toml yet (first session). Build check skipped."
fi
echo ""
...
You are yoyo, a coding agent growing the zellai project. Today is $DATE $SESSION_TIME.
...
1. **Read project context** — YOYO.md (project goals, tech stack), SCHEMA.md (data contracts), zellai-vision.md (founding vision)
...
5. **Check build** — If Cargo.toml exists: run \`cargo build --target wasm32-wasip1\` and \`cargo test --lib\`. Note what passes/fails.
...
## Build Status
[pass/fail from cargo build + cargo test --lib, or "no Cargo.toml yet"]

## Project State
[what exists so far — plugin modules, features, event handlers]
...
## Gaps & Opportunities
[what's missing relative to YOYO.md vision and zellai-vision.md product description]
...
Read the codebase yourself: YOYO.md, SCHEMA.md, zellai-vision.md, src/ (if exists), Cargo.toml."
fi
...
Include which files to create, what components to build, what plugin events to handle.
Mention build/test verification: cargo build --target wasm32-wasip1 && cargo clippy --target wasm32-wasip1 && cargo test --lib]

TASK SIZING RULES:
- Each task should be completable in 20 minutes
- Each task must touch at most 5 files
- Each task must be independently verifiable (cargo build passes)
...
Read YOYO.md, SCHEMA.md, and zellai-vision.md. Identify the most impactful improvement and implement it.
...
SAFETY:
- NEVER modify zellai-vision.md, SCHEMA.md, .github/workflows/, .yoyo/scripts/, or core skills
- NEVER delete tests
- NEVER use std::fs, std::net, or std::process in plugin code (WASM sandbox)
- NEVER block in render()
TEOF
...
Continue from the committed state. Do NOT redo committed work."
    ...
    BUILD_FIX_ATTEMPT=0
    MAX_BUILD_FIX=5
    while [ "$TASK_OK" = true ] && [ -f Cargo.toml ]; do
        BUILD_FAILED=""
        BUILD_OUT=""
        TEST_OUT=""
        ...
        elif ! TEST_OUT=$(cargo test --lib 2>&1); then
            BUILD_FAILED="tests"
...
Fix the specific errors. After fixing: cargo build --target wasm32-wasip1 && cargo clippy --target wasm32-wasip1 && cargo test --lib
BFIXEOF
        BFIX_LOG=$(mktemp)
...
2. Check the plugin renders correctly (no blocking in render())
3. Check for obvious bugs tests don't catch
4. Verify no forbidden APIs: std::fs, std::net, std::process must not appear in plugin code
...
FAIL only for: wrong implementation, obvious bugs, security issues, forbidden API usage.
...
Fix the issues. After fixing: cargo build --target wasm32-wasip1 && cargo clippy --target wasm32-wasip1 && cargo test --lib
FIXEOF
                FIX_LOG=$(mktemp)
                ...
                if [ -f Cargo.toml ]; then
                    if ! cargo build --target wasm32-wasip1 2>&1 >/dev/null; then
                        TASK_OK=false
...
You are yoyo, growing the zellai project. You just finished a growth session ($DATE $SESSION_TIME).
...
You are yoyo, growing the zellai project. You just finished a session ($DATE $SESSION_TIME).
...
You are yoyo, growing the zellai project. You finished a growth session ($DATE $SESSION_TIME).
